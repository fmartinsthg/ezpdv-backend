...
async function main() {
  console.log(
    "=== Cleanup de Categorias (poupa categorias com produtos protegidos/Recipe) ==="
  );
  ...
  console.log(
    `Produtos protegidos (por Recipe): ${protectedProductIds.length}`
  );
  ...
  const keepCategoryIds = new Set(
    categoriesForProtectedProducts
      .map((p) => p.categoryId)
      .filter((id): id is string => !!id)
  );
  console.log(
    `Categorias a manter (por abrigarem produtos protegidos): ${keepCategoryIds.size}`
  );
  ...
  const deleteCategoryIds = [...allCategoryIds].filter(
    (id) => !keepCategoryIds.has(id)
  );
  ...
  const keptWithParentToDelete = allCategories.filter(
    (c) =>
  ...
  console.log(
    `Categorias mantidas com parentId a ser deletado: ${keptWithParentToDelete.length}`
  );
  ...
  console.log(
    `Produtos que serão desanexados das categorias a deletar (não-protegidos): ${productIdsToNullCategory.length}`
  );
  ...
  if (stillPointing > 0) {
    console.warn(
      `ABORTADO: ainda existem ${stillPointing} produtos apontando para categorias candidatas à deleção.`
    );
    console.warn(
      `Revise vínculos antes de prosseguir (há produtos fora do escopo ou protegidos vinculados a categorias candidatas).`
    );
    return;
  }
