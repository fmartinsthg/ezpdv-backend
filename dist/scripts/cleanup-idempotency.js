...
async function purgeExpired() {
    ...
    const where = {
        ...
        status: {
            in: [
                client_1.IdempotencyStatus.SUCCEEDED,
                client_1.IdempotencyStatus.FAILED,
                client_1.IdempotencyStatus.EXPIRED,
            ],
        },
    };
    const { count } = await prisma.idempotencyKey.deleteMany({ where });
    console.log(`PURGE-EXPIRED: deletados=${count} (retentionDays=${RETENTION_DAYS})`);
}
async function forceRetryStuck() {
    ...
    console.log(`FORCE-RETRY-STUCK: marcados=${count} (processing>${STUCK_MINUTES}min)`);
}
async function nukeScope(scope) {
    mustTenant();
    if (!scope) {
...
async function main() {
    ...
    switch (cmd) {
        ...
        default:
            console.log("Uso:");
            console.log("  npx tsx scripts/cleanup-idempotency.ts purge-expired    # apaga SUCCEEDED/FAILED/EXPIRED antigos");
            console.log("  npx tsx scripts/cleanup-idempotency.ts force-retry-stuck # marca PROCESSING antigos como FAILED");
            console.log("  npx tsx scripts/cleanup-idempotency.ts nuke-scope orders:create # DEV: apaga tudo do scope");
    }
}
